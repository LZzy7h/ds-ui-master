<template>
  <div>
    <div
      class="main_content"
      style="border-top-right-radius: 0;border-top-left-radius: 0; padding-top: 40px;"
    >
      <el-form ref="form" label-width="120px">
        ...
        <el-form-item label="算法名称">
            <a class="pl15" @click="codeDialogVisible=true" style="text-decoration:underline;">{{ contractDetail.algorithmName }}</a>
        </el-form-item>
        ...
        <el-form-item label="剩余执行次数">
          <span class="pl15">{{ contractDetail.leftAmount }}次</span>
        </el-form-item>
        <el-form-item label="已执行任务">
          <el-table :data="tableData" class="pl15" border style="width: 80%">
          ...
          <div class="pagination-wrap" style="width: 80%" v-if="total">
            <pagination
              v-show="total > 0"
              :total="total"
              :page.sync="pageNo"
    ...
    >
      <div>
        <el-input type="textarea" :rows="15" v-model="contractDetail.algorithmCode"></el-input>
      </div>
    </el-dialog>
  </div>
</template>
<script>
import { getOperatorContractDetails } from "@/api/contract";
...
export default {
  ...
  methods: {
    getContractDetailsFunc() {
      getOperatorContractDetails(this.id).then(res => {
        if (res.code === 200) {
