<template>
  <div class="applicationList">
    <div class="search_box" v-show="showSearch">
      <el-form :model="search" :inline="true" >
        ...
        <el-form-item class="searchButton">
          <el-button
            type="primary"
            @click="getOperatorDatasetCatalogFunc"
          >搜索</el-button
          >
          <el-button
            @click="handleReset"
    ...
    <div class="operation_btn" style="margin-top:20px;line-height: 36px">
      ...
      <right-toolbar :showSearch.sync="showSearch" @queryTable="getOperatorDatasetCatalogFunc"></right-toolbar>
    </div>
    <div class="main_content category">
      <el-table
        ...
        row-key="id"
        default-expand-all
        :tree-props="{children: 'childrenDatasetCatalogs', hasChildren: 'hasChildren'}">
        <el-table-column
          prop="categoryName"
          label="目录名称">
        </el-table-column>
...
<script>
  import { getOperatorDatasetCatalog, addDataDirectory, delDataDirectory, updateDataDirectory } from '@/api/ScenarioSetting'
  ...
  export default {
    ...
    mounted(){
      this.getOperatorDatasetCatalogFunc()
    },
    methods: {
      getOperatorDatasetCatalogFunc(){
        let data = {
          categoryName: this.search.categoryName || undefined
        }
        getOperatorDatasetCatalog(data).then((res) => {
          if (res.code === 200) {
            this.tableData = res.data
            this.option[0].childrenDatasetCatalogs = res.data
          }
        })
      },
      resetForm(){
      ...
      deleteCategory(row){
        this.$confirm('确认删除本节点?', '', {
          ...
          delDataDirectory(row.id).then((res) => {
            if (res.code === 200) {
              this.$message({
                type: 'success',
                message: '删除成功!'
      ...
      handelSave(){
        ...
        if (this.isAdd) {
          addDataDirectory(this.form).then((res) => {
            if (res.code === 200) {
          ...
          updateDataDirectory(this.form).then((res) => {
            if (res.code === 200) {
              this.$message.success('修改数据目录成功！')
              this.dialogVisible = false
              this.getOperatorDatasetCatalogFunc()
            }
          })
