<template>
    ...
    <div class="main_content" style="border-top-right-radius: 0;border-top-left-radius: 0;padding-left: 35px;">
      <div class="param_box pt30">
        ...
        <el-upload
          ...
          :limit="1"
        >
          <el-button size="small" type="primary">选择本地源文件</el-button>
        </el-upload>
      </div>
      ...
      <div class="param_box pt30">
        <span class="param_box_label">算法名称：</span>
 ...
 <el-input
      ...
      <div class="param_box pt30">
        <span class="param_box_label">应用目录：</span>
        <div style="position:relative;">
          <el-select
            v-model="scenarioId"
            placeholder="请选择"
            @change="checkscenarioId(scenarioId)"
            clearable
            style="width: 300px"
          >
            <el-option
    ...
    <el-dialog
      ...
      class="ide-filepath dialog-middle"
      @close="handleCloseDialog"
      center
    >
      <div class="ide-filepath-return flex" v-if="directory.length > 0">
        <i class="el-icon-arrow-left"></i>
        <span @click="handleReturn">上一级</span>
        <span>{{ directory[directory.length - 1] }}</span>
      </div>
      <div class="ide-filepath-root flex" v-if="directory.length === 0">
        <span>根目录</span>/
      </div>
      <!--@current-change="handleCurrentChange"-->
      <el-table
        :data="tableData"
        @row-click="handleOpenDialog"
      ...
      >
        <el-table-column prop="name" label="名称">
          <template slot-scope="scope">
            <div>
              <img
                :src="scope.row.type === 1 ? fileImg : folderImg"
                class="file-icon"
              />
      ...
      <span slot="footer" class="dialog-footer">
        ...
        <el-button type="primary" @click="handleGetFileContent"
          >确 定</el-button
        >
      </span>
    </el-dialog>
...
import { addAlgorithm, getFileList, getFileContent } from "@/api/algorithm";
...
export default {
  ...
  data() {
    return {
      ...
      file: "",
      fileImg: require("../../../assets/file.png"),
  ...
  methods: {
    ...
    handleOpenDialog(row) {
      const filePath = row.curDir ? row.curDir : row;
      if (row.type === 1) {
        this.file = filePath;
      } else {
        getFileList({ name: filePath }).then(res => {
          if (res.code === 200) {
            this.tableData = res.data;
            if (!this.centerDialogVisible) this.centerDialogVisible = true;
            if (row.lastDir) {
              this.directory.push(row.lastDir);
            } else {
              this.directory.pop();
            }
          }
    ...
    handleGetFileContent() {
      getFileContent({ name: this.file }).then(res => {
        if (res.code === 200) {
    ...
    handleReturn() {
      let data = this.directory[this.directory.length - 1];
