...
<script>
import { getProviderContractDetails } from "@/api/contract";
import {
  getExecutionRecordListForData,
  queryFailReason
...
export default {
  ...
  methods: {
    getContractDetailsFunc() {
      getProviderContractDetails(this.id).then(res => {
        if (res.code === 200) {
    ...
    getExecutionRecordListFunc() {
      ...
      getExecutionRecordListForData(data).then(res => {
        if (res.code === 200) {
